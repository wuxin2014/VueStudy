<template>
  <div class="list-content" @click="handleTop" ref="wrapper">
    <van-pull-refresh
...
export default {
  ...
  filters: {
    transDate(longTime) {
      ...
      // 时间差小于1,相隔一天显示昨天, 或者相隔一月
      if (dayDiff < 1 && (nowDay - day === 1 || mouth + 1 === nowMouth)) {
        dayDiff = 1;
      }
          ...
          if (this.orientation === 'UP') {
            ...
            this.$nextTick(() => {
              this.$refs.wrapper.scrollTo(
                0,
                this.$refs.wrapper.scrollHeight - this.contentHeight
              );
              this.contentHeight = this.$refs.wrapper.scrollHeight;
            });
          } else {
            this.list = newResList;
            this.$nextTick(() => {
              const contentHeight = this.$refs.wrapper.scrollHeight;
              this.$refs.wrapper.scrollTo(0, contentHeight - 10);
